...
import { getTopProductsByRevenue } from '../get-top-products-by-revenue';
import {
  setRequest,
  captureRequest,
  URLS,
...
const RAW_ITEM = { product_name: 'Consultoría', total_revenue: '15000.00' };

describe('[API] getTopProductsByRevenue', () => {
  describe('filter parameters', () => {
    test('sends limit when provided', async () => {
      const captured = captureRequest(URLS.ANALYTICS.TOP_PRODUCTS_BY_REVENUE);
      await getTopProductsByRevenue(TEST_BATCH_ID, 10);
      expect(captured.params!.get('limit')).toBe('10');
    });

    test('omits limit when not provided', async () => {
      const captured = captureRequest(URLS.ANALYTICS.TOP_PRODUCTS_BY_REVENUE);
      await getTopProductsByRevenue(TEST_BATCH_ID);
  ...
  describe('auth token', () => {
    test('sends Authorization header when authToken is provided', async () => {
      const captured = captureRequest(URLS.ANALYTICS.TOP_PRODUCTS_BY_REVENUE);
      await getTopProductsByRevenue(TEST_BATCH_ID, undefined, { authToken: 'test-token' });
    ...
    test('omits Authorization header when authToken is null', async () => {
      const captured = captureRequest(URLS.ANALYTICS.TOP_PRODUCTS_BY_REVENUE);
      await getTopProductsByRevenue(TEST_BATCH_ID, undefined, { authToken: null });
      expect(captured.headers!.get('Authorization')).toBeNull();
    });
  });
  ...
  describe('return value', () => {
    test('maps response to camelCase', async () => {
      setRequest({
        url: URLS.ANALYTICS.TOP_PRODUCTS_BY_REVENUE,
        body: { batch_id: TEST_BATCH_ID, data: [RAW_ITEM] },
      });
      const result = await getTopProductsByRevenue(TEST_BATCH_ID);
      expect(result.batchId).toBe(TEST_BATCH_ID);
      expect(result.data[0]).toEqual({ productName: 'Consultoría', totalRevenue: '15000.00' });
    });
    ...
    test('does not expose snake_case keys', async () => {
      ...
      expect(result.data[0]).not.toHaveProperty('total_revenue');
    });
